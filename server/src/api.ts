...
export default (ws: WebSocket) => {
  function new_game(creator: string, number_of_players: number): IndexedGame | PendingGamesGame {
  ...
  function PendingGames_games(): Readonly<PendingGamesGame[]> {
    return G.all_PendingGames_games()
  }

  function join(id: number, player: string): IndexedGame | PendingGamesGame {
    return G.join(id, player)
  }
  
  function broadcast(game: IndexedGame | PendingGamesGame): void {
