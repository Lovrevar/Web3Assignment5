...
import { IndexedGame, PendingGamesGame } from './servermodel'
...
function start_server(ws: WebSocket) {
  ...
  gameserver.post('/PendingGames-games', async (req: TypedRequest<{creator: string, number_of_players: number}>, res: Response<PendingGamesGame|IndexedGame>) => {
  ...
  gameserver.get('/PendingGames-games', async (_: Request, res: Response<Readonly<PendingGamesGame[]>>) => {
    const games = api.PendingGames_games()
    res.send(games)
  })

  gameserver.get('/PendingGames-games/:id', async (req: Request, res: Response<PendingGamesGame>) => {
    const games = api.PendingGames_games()
  ...
  gameserver.get('/PendingGames-games/:id/players', async (req: Request, res: Response<string[]>) => {
    const games = api.PendingGames_games()
    const g = games.find(g => g.id === parseInt(req.params.id))
    if (!g)
      res.status(404).send()
  ...
  gameserver.post('/PendingGames-games/:id/players', async (req: TypedRequest<{player: string}>, res: Response<PendingGamesGame|IndexedGame>) => {
